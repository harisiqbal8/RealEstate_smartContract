...
describe("Real Estate", function () {
  ...
  it("Get Admin Value: ", async()=>{
    const [propertyNo ,commsion , fee] = await property.getAdmin();
  ...
  it("Get Property Data: ", async()=>{
    const [pics, pprs, taxes, mortgage, date, pamount, owner, sale] = await property.getData(1);
    console.log(pics, pprs, taxes, mortgage, date, pamount, owner, sale);
  });
